...
import { loadTileServiceData, getEntityPatterns } from './tile-data.js'
...
interface ScorerInput {
  ...
  serviceType?: string       // For topical authority link validation
...
function scoreStyleAdherence(input: ScorerInput, root: HTMLElement, fullText: string): CategoryScore {
  ...
  switch (style) {
    ...
    default: {
      ...
      // text-based labels OR structural detection (early list after first paragraph)
      const hasTakeawaysText = /key takeaway|takeaway|at a glance|quick summary|key points|key facts|highlights/i.test(fullText)
      const topElements = root.childNodes.filter((n) => n instanceof HTMLElement).slice(0, 8) as HTMLElement[]
      let hasEarlyListStructural = false
      let seenParagraph = false
      for (const el of topElements) {
        if (el.tagName === 'P') seenParagraph = true
        if (seenParagraph && (el.tagName === 'UL' || el.tagName === 'OL')) {
          hasEarlyListStructural = true
          break
        }
      }
      const hasTakeaways = hasTakeawaysText || hasEarlyListStructural
      checks.push({
...
function scoreExternalLinkValidation(input: ScorerInput, root: HTMLElement): CategoryScore {
  ...
  if (input.serviceType && externalLinks.length > 0) {
    const approved = getApprovedDomains(input.serviceType)
    let approvedCount = 0
    ...
    checks.push({
      ...
      maxPoints: 3,
      detail: input.serviceType ? 'No external links to validate' : 'No service type — skipping domain validation',
    })
  }
...
function scoreCtaRelevance(input: ScorerInput): CategoryScore {
  const checks: Check[] = []
  // Include keyword words AND service type for matching
  const kwWords = input.keyword.toLowerCase().split(/\s+/).filter(w => w.length > 2)
  const serviceType = input.serviceType?.toLowerCase() || ''

  // Map service types to common aliases
  const serviceAliases: Record<string, string[]> = {
    bathroom: ['bathroom', 'bath'],
    shower: ['shower', 'walk-in'],
    floor: ['floor', 'flooring'],
    backsplash: ['backsplash', 'kitchen'],
    repair: ['repair', 'fix', 'restoration'],
    reglazing: ['reglazing', 'refinishing', 'resurfacing'],
  }
  const aliases = [serviceType, ...(serviceAliases[serviceType] || [])]

  // Check if CTAs reference the article topic — match on keyword words OR service type
  const earlyCta = input.earlyCta?.toLowerCase() || ''
  const closingCta = input.closingCta?.toLowerCase() || ''
  const earlyHasKeyword = kwWords.some(w => earlyCta.includes(w)) || aliases.some(a => a && earlyCta.includes(a))
  const closingHasKeyword = kwWords.some(w => closingCta.includes(w)) || aliases.some(a => a && closingCta.includes(a))
  ...
  checks.push({
    ...
    detail: ctaRelevant
      ...
      : `CTAs don't reference keyword terms or service type: ${[...kwWords, serviceType].filter(Boolean).join(', ')}`,
  })
