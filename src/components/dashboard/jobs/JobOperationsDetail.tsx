...
// The operations detail for a job past the sales pipeline. Rendered by the
// unified /dashboard/leads/[id] wrapper for accepted → paid jobs. It is never
// rendered for lead-stage jobs, so the old lead-stage redirect is gone.
export default async function JobOperationsDetail({ id }: { id: string }) {
  let job: Job
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isOwner && (
              <Link
                href={`/dashboard/leads/${job.id}/estimate`}
